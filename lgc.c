...
static void reallymarkobject(global_State *g, GCObject *o)
{
  ...
  case LUA_VPROTO:
  {
    fprintf(log_file, "gcoplog:reallymarkobject link obj %p to gray list\n", o);
...
static void traverseweakvalue(global_State *g, Table *h)
{
  ...
  {
    fprintf(log_file, "gcoplog:traverseweakvalue link obj(table 弱v表) %p to gray list for g->gcstate==Gcatimic \n", h);
    linkgclist(h, g->weak); /* has to be cleared later */
  }
  else // 否则其实就是传播阶段吧
  {
    fprintf(log_file, "gcoplog:traverseweakvalue link obj(table 弱v表) %p to grayagain list \n", h);
    linkgclist(h, g->grayagain); /* must retraverse it in atomic phase */
  }
}

/*
   ** 遍历一个瞬表（ephemeron短暂临时 table  weak key表），并将其链接到适当的列表中。
   ...
   ** 弱key, 强value 那如果是同一张表里的value引用了 key呢

   ** key的连通性，决定了value的连通性，但是value的连通性并不影响key的连通性
*/
static int traverseephemeron(global_State *g, Table *h, int inv)
{
  ...
  {
    fprintf(log_file, "gcoplog:traverseephemeron link obj(table 弱k表) %p to grayagain list in GCSpropagate state \n" ,h);
    linkgclist(h, g->grayagain); /*  原子阶段需重新遍历 */
  }
  else if (hasww) /* 存在 "白色键 -> 白色值? */
  {
    fprintf(log_file, "gcoplog:traverseephemeron link obj(table 弱k表) %p to ephemeron list because has ww \n" ,h);
    linkgclist(h, g->ephemeron); /* have to propagate again 需再次传播 */
  }
  else if (hasclears) /* table has white keys? 存在白色键 */
  {
    fprintf(log_file, "gcoplog:traverseephemeron link obj(table 弱k表) %p to allweak list because has hasclear \n" ,h);
...
static lu_mem traversetable(global_State *g, Table *h)
{
  ...
  {                               /* is really weak? */
    ...
    {
      fprintf(log_file, "gcoplog:traversetable link obj(table 弱kv表) %p to allweak list \n" ,h);
...
static int traversethread(global_State *g, lua_State *th)
{
  ...
  { 
    fprintf(log_file, "gcoplog:traversethread link obj(lua_State) %p to grayagain list in GCSpropagate state \n" ,th);
...
static lu_mem propagatemark(global_State *g)
{
  ...
  fprintf(log_file, "gcoplog:propagatemark pop obj %p from gray list ,and turn it to black\n", o);
  switch (o->tt)
  {
  case LUA_VTABLE:
...
** 遍历所有 Ephemeron Tables，将标记（mark）从键（keys）传播到值（values）
** 解释：
Ephemeron Table 的键是弱引用（weak keys），但值可能是强引用。
在垃圾回收的标记阶段，如果一个键被标记为“存活”（即它被其他强引用持有），那么它的关联值也必须被标记为“存活”。
这一步骤确保 键和值的依赖关系 被正确处理

** Repeat until it converges, that is, nothing new is marked.
含义：重复上述过程，直到收敛（即没有新的对象被标记）。
解释：
Ephemeron Tables 可能存在 间接依赖链。例如：
表 A 的键是 k1，值是 v1。
表 B 的键是 v1，值是 v2。
如果 k1 被标记，v1 需要被标记，进而 v2 也需要被标记。
这种依赖可能需要多次遍历才能完全传播标记，直到没有新的对象被标记（即收敛

** 'dir' inverts the direction of the traversals, trying to speed up
** convergence on chains in the same table.
含义：dir 参数反转遍历方向（正向/反向），以加速同一表中依赖链的收敛。
解释：
某些 Ephemeron Tables 可能存在 长依赖链（如 k1 → v1 → k2 → v2 → ...）。
单向遍历可能导致标记传播缓慢（类似“波浪式”推进）。
通过交替切换遍历方向（如正向一次、反向一次），可以更快地覆盖所有依赖路径，加速收敛。
这是优化手段，类似图的广度优先搜索（BFS）中调整遍历顺序。


整体逻辑总结
标记阶段：Lua GC 首先标记所有强引用可达的对象。
处理 Ephemeron Tables：
遍历所有 Ephemeron Tables，检查键是否被标记：
如果键被标记，则标记其关联值。
重复遍历，直到没有新对象被标记（确保所有间接依赖被处理）。
使用 dir 方向切换优化长依赖链的标记传播效率。
清理阶段：回收所有未被标记的对象（弱引用键/值会被清除）。

** 这段代码注释描述的是 Lua 垃圾回收器（GC） 中处理 Ephemeron Tables（短暂表） 的标记阶段逻辑。
** 它的核心目的是解决 Ephemeron Tables 在垃圾回收过程中可能出现的 依赖循环问题，确保所有可达对象被正确标记，同时避免内存泄漏或误回收
*/
                            ...
                            int *countout)
{
  ...
  for (i = 0; *p != NULL && i < countin; i++)
  {
    ...
    {                   /* &运算大于0 说明是同一种白那么就要删除 为被标记 is 'curr' dead?  对象是否未被标记*/
      *p = curr->next;  /* remove 'curr' from list  从链表中移除对象*/
      fprintf(log_file, "gcoplog:sweeplist free dead obj %p !!!!!!!!!!!!!!!!!!!!!!\n", curr);
      freeobj(L, curr); /* erase 'curr'  释放对象内存*/
    }
    else
    {                                                           /* change mark to 'white' 否则是被标记成黑色或者另一种白,然后要把marked统一搞成另一种白*/
      curr->marked = cast_byte((marked & ~maskgcbits) | white); // 清0再或上另一种白
      p = &curr->next;                                          /* go to next element */
      fprintf(log_file, "gcoplog:sweeplist change obj %p marked  !!!!!!!!!!!!!!!!!!!!!!\n", curr);
    }
  }
  if (countout)
...
/*
** 因为他们不可达，所以他们的finalization需要被调用，这就是gc
...
** mt参数就是要设置的元表，如果元表里有__gc方法就会把这个obc从allgc移动到finobj上
*/
...
static lu_mem atomic(lua_State *L)
{
  ...
  fprintf(log_file, "gcoplog:atomic mark root again..................\n");
  ...
  fprintf(log_file, "gcoplog:atomic remarkupvals ..................\n");
  work += remarkupvals(g); // 重新标记可能死亡的线程的 upvalue
  fprintf(log_file, "gcoplog:atomic empty gray list..................\n");
  work += propagateall(g); /* propagate changes */
  g->gray = grayagain;     // 恢复 grayagain 列表
  fprintf(log_file, "gcoplog:atomic empty grayagain list..................\n");
  work += propagateall(g); /* traverse 'grayagain' list */
  fprintf(log_file, "gcoplog:atomic first time deal ephemeron list........\n");
...
static lu_mem singlestep(lua_State *L)
{
  ...
  case GCSpropagate:
  {
    if (g->gray == NULL)
    {                              /* 灰色对象链表为空？ */
      fprintf(log_file, "gcoplog:singlestep gray list is null turn state to atomic\n");
...
#define GCScallfin 7
#define GCSpause 8
const char *GCStateStr[] = {"GCSpropagate", "GCSenteratomic", "GCSatomic", "GCSswpallgc", "GCSswpfinobj", "GCSswptobefnz", "GCSswpend", "GCScallfin", "GCSpause"};
// 打印gc链表的一些函数
void ShowGcList(global_State *g, const char *title, GCObject *list)
{
  ...
  for (GCObject *o = list; o != NULL; o = o->next)
  {
    ...
    if (iswhite(o))
      fprintf(log_file, "white %p)", o);
    else if (isgray(o))
      fprintf(log_file, "gray %p)", o);
    else if (isblack(o))
      fprintf(log_file, "black %p)", o);
    else
...
void ShowGcState(global_State *g, const char *log_str,int count)
{
  if (log_file == NULL)
  {
  ...
  fprintf(log_file, "%s %d gcstate: %s\n", log_str,count, GCStateStr[g->gcstate]);
  ShowGcList(g, "allgc", g->allgc);
  ShowGcList(g, "finobj", g->finobj);
  ShowGcList(g, "tobefnz", g->tobefnz);
  //ShowGcList(g, "fixedgc", g->fixedgc); 这里存着一些不会被销毁的，可以不用打印
