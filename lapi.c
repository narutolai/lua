...
LUA_API int lua_getiuservalue (lua_State *L, int idx, int n) {
  ...
  if (n <= 0 || n > uvalue(o)->nuvalue) {
    setnilvalue(s2v(L->top.p));    // 越界时压入 nil
    t = LUA_TNONE;                 // 返回 LUA_TNONE
  }
  else {
    setobj2s(L, L->top.p, &uvalue(o)->uv[n - 1].uv);//设置到栈顶啊
    t = ttype(s2v(L->top.p));         //返回值类型
  }
  api_incr_top(L);                //L->top.p++
  lua_unlock(L);
  return t;
}
...
//lua_KContext ctx该类型必须是一个 数值类型（如整数类型），因为 Lua 需要用它来存储指针或偏移量等底层信息
LUA_API int lua_pcallk (lua_State *L, int nargs, int nresults, int errfunc,
  ...
  if (k == NULL || !yieldable(L)) {  /* no continuation or no yieldable? */
    ...
    ci->u.c.ctx = ctx;  /* save context 这个是个index 2*/
